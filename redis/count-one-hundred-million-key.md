...
可以看到，这个差集中的用户 ID 在 user-id-20200804 的 Set 中存在，但是不在累计用户 Set 中。
...
在介绍具体的方法之前，我们要先知道，Bitmap 支持用 BITOP 命令对多个 Bitmap 按位做“与”“或”“异或”的操作，
...
有一点需要注意一下，HyperLogLog 的统计规则是基于概率完成的，所以它给出的统计结果是有一定误差的，标准误算率是 0.81%。
